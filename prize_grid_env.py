...
class GridWorld():
    ...
    def weighted_traj_return(self, mat_state, type="M"):
        if self.env_params["node_weight"] == "constant" or self.env_params["node_weight"] == "gorilla" or self.env_params["node_weight"] == "GP" or self.env_params["node_weight"] == "bimodal":
            ...
            return self.ret_steiner_covering(mat_state, type)
    ...
    def ret_steiner_covering(self, mat_state, type):
        traj = torch.vstack(mat_state)
        returns = []
        self.steiner_map = []
    ...
    def get_prize_cnt(self, mat_state, type="steiner"):
        assert self.env_params["node_weight"] == "steiner_covering"

        traj = torch.vstack(mat_state)
        prize_return = torch.zeros(len(self.items_loc))

        for k, item in enumerate(self.items_loc):
            item_coverage = self.items_loc[item].intersection(set(traj[:, 0].tolist()))
            num_items = len(item_coverage)
            prize_return[k] = min(num_items, self.Dmin[k])
...
# OpenAI gym wrapper fo the Grid World environment
class GridWorldGym(gym.Env):
    def __init__(self, env_params, common_params, visu_params, alg_params, env_file_path):
        super(GridWorldGym, self).__init__()

        self.alg_params = alg_params

        self.env = GridWorld(env_params=env_params, common_params=common_params, visu_params=visu_params, env_file_path=env_file_path)
    ...
    def step(self, action):
        ...
        batch_state = self.mat_state[-1].reshape(-1, 1).float()
        batch_state = torch.cat(
            [batch_state, self.t * torch.ones_like(batch_state)], 1) # append time index to the state

        self.mat_return.append(self.env.weighted_traj_return(self.mat_state, type = self.alg_params["type"]))
        marginal_reward = self.mat_return[-1] - self.mat_return[-2] # get the difference of the trajectories
